...
class FarnebackFlowField:
    ...
    (PRE e POST), insieme alla maschera dei pixel considerati affidabili: quelli che
    ricadono sui vasi in entrambe le fasi e il cui spostamento non supera la soglia di
    movimento massimo plausibile (probabile rumore oltre quella soglia).
    """

    def __init__(self, img_pre: np.ndarray, img_post: np.ndarray, vessel_mask_pre: np.ndarray,
                 vessel_mask_post: np.ndarray, max_allowed_movement: float):
        ...
        vessel_mask = vessel_mask_pre.astype(bool) & vessel_mask_post.astype(bool)
        magnitude_mask = self.magnitude < max_allowed_movement
        self.valid_mask = vessel_mask & magnitude_mask
    ...
    def valid_points(self):
        ...
        xx_valid = xx[self.valid_mask]
        yy_valid = yy[self.valid_mask]

        valid_coordinates = np.column_stack([xx_valid, yy_valid])
        valid_values = self.raw_field[self.valid_mask]
